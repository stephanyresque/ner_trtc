"""Experiência D: extrai os 5 campos a partir do TEXTO via modelo local de texto (vLLM), só nos texto_real.

Retoma execuções anteriores por padrão (--do-zero refaz tudo); --limit N para um teste rápido.
Rode a partir de ner_trct/:  python -m experimentos.d_texto_local.run
"""
...
from experimentos.d_texto_local.extrair_texto_local import CAMPOS, _config, extrair_campos

SAIDAS = Path(__file__).resolve().parent / "saidas"
CLASSES_D = {"texto_real"}            # escopo D: só docs com camada de texto útil


def _slug(modelo: str) -> str:
    return modelo.rsplit("/", 1)[-1]
...
def _montar_resumo(modelo: str, docs: list, itens: list, t_run: float) -> dict:
    ...
    return {
        "experimento": f"D — texto, modelo local via vLLM ({modelo})",
        "modelo": modelo,
        "modalidade": "texto",
        "custo_total_usd": 0.0,                 # modelo local
...
def main() -> None:
    ...
    docs = [d for d in triagem if d.get("classe") in CLASSES_D]
    if args.limit:
        docs = docs[:args.limit]

    pred_path = SAIDAS / f"predicoes_D_{slug}.json"
    ...
    for i, d in enumerate(docs, start=1):
        ...
        if not args.do_zero and b in feitos_meta and b in feitos_rec:
            ...
            try:
                ...
                meta = {"usage": None, "custo_usd": 0.0, "latencia_s": None, "modo": None,
                        "validacao": None, "erro": str(e), "n_chars": 0}
                print(f"    ERRO: {e}")
        predicoes[b] = rec
    ...
    print(f"\n{'='*60}")
    print(f"  Rodados: {len(docs)} | com erro: {resumo['n_docs_com_erro']} | custo: R$ 0,00 (modelo local)")
    ...
    print(f"Próximo: python -m pipeline.run_compare --fonte json "
          f"--pred experimentos/d_texto_local/saidas/predicoes_D_{slug}.json "
          f"--out relatorios/d/comparacao_D_{slug}.json")
