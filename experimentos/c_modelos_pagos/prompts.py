"""Prompts da Experiência C: extração dos 5 campos do TRCT a partir das imagens das páginas.

Mesmas âncoras de campo da B (formulário TRCT padronizado), redigidos para várias páginas,
já que a C envia TODAS as páginas do PDF (não só a 1ª).
"""

SYSTEM_PROMPT = (
    "Você extrai dados de TRCT (Termo de Rescisão do Contrato de Trabalho) a partir das imagens "
    "das páginas do formulário e responde APENAS com um JSON. Nunca invente: campo ausente = string vazia."
)

USER_PROMPT = (
    "As imagens são as páginas de um formulário TRCT padronizado, com campos numerados. Extraia "
